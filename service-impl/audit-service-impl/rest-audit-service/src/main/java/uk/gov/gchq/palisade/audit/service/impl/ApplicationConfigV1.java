...
public class ApplicationConfigV1 extends AbstractApplicationConfigV1 {
    private static final Class<?>[] RESOURCES = new Class<?>[]{
            RestAuditServiceV1.class
    };

    public ApplicationConfigV1() {
        super(RESOURCES);
        //make sure we can inject the service instance
        AuditService delegate = RestAuditServiceV1.createService(System.getenv(ConfigConsts.CONFIG_SERVICE_PATH));
        register(new ServiceBinder(delegate, AuditService.class));
    }
}
